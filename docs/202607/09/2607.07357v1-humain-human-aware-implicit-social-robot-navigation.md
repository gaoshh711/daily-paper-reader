...
tldr: "社交机器人导航需感知人类行为，但现有方法未充分利用隐式线索。本文提出HumAIN框架，通过知识蒸馏将多模态教师模型（融合图像、骨骼、状态与目标）的社交感知能力传递给轻量学生模型，使其仅用最少输入即可推理社交动态。实验显示，轨迹预测指标平均提升29.8%，证明隐式全身线索能提升资源受限平台的导航类人性。"
source: arxiv
...
motivation: 现有社交导航忽略人类隐式骨骼线索（如步态、朝向），导致规划不够自然。
method: 采用transformer教师模型融合多模态输入，再通过轨迹重建与特征对齐蒸馏至轻量学生模型。
result: "轨迹预测指标平均提升29.8%，优于所有基线。"
conclusion: HumAIN使机器人能高效利用隐式社交线索，实现资源受限平台上的类人导航。
---

## 摘要
有效的社交机器人导航需要对人类行为的敏感性，这些行为常常通过微妙的骨骼线索（如步态和朝向）显现。我们提出了人类感知的隐式社交机器人导航（HumAIN），这是一种新颖的框架，通过知识蒸馏将隐式社交线索直接融合到规划循环中。我们首先采用基于变换器的教师模型，该模型融合丰富的多模态输入（包括历史图像、骨骼关键点、机器人状态和机器人的目标），为机器人的未来轨迹规划学习鲁棒的、人类感知的表示。为了实现实时部署，我们随后将这些知识蒸馏到一个轻量级学生模型中。通过同时优化轨迹重建和与教师的潜在特征对齐，学生模型学会从最小输入推断复杂的社会动态。通过高效的蒸馏架构弥合预测与规划之间的差距，我们的方法使机器人能够以自适应、鲁棒且符合社会规范的方式推理人类行为。我们通过大量实验验证了HumAIN，与最先进的基线相比，它在所有轨迹预测指标上平均提升了29.8%。这些结果凸显了在资源受限平台上利用隐式全身线索实现类人导航感知的优势。
